...
import { calcTest } from '../services/api'
...
export default function Home() {
  ...
  const LOADING_MESSAGES = ['사주를 분석하고 있어요','명반을 그리는 중이에요','운세를 정리하고 있어요','거의 다 됐어요']

  useEffect(() => {
    if (!loading) return
    const interval = setInterval(() => setLoadingMsg(prev => (prev + 1) % LOADING_MESSAGES.length), 1800)
  ...
  useEffect(() => {
    function calc() {
      const target = new Date('2026-11-19'); target.setHours(0,0,0,0)
  ...
  function buildInput() {
    ...
    return {
      ...
      calendar: form.calendar, is_leap: form.is_leap,
    }
  }
  ...
  async function handlePay(serviceType) {
    ...
    try {
      ...
      const { data: order } = await axios.post('/api/payment/order', {
        service_type: serviceType, cache_key: cacheRes.cache_key,
      })
      sessionStorage.setItem('ziwei_input', JSON.stringify(input))
      ...
      if (!response || response.code) {
        setErr(response?.message || '결제가 취소되었습니다.')
        setLoading(null)
        return
      }
      ...
      const { data: confirmed } = await axios.post('/api/payment/confirm', {
        ...
        cache_key: cacheRes.cache_key,
      })
      sessionStorage.setItem('payment_key', confirmed.payment_key)
      sessionStorage.setItem('cache_key', confirmed.cache_key)
      nav('/result')
    ...
    <div style={s.page}><div style={s.card}>
      ...
      <div style={s.hero}>
        <div style={s.heroRow}>
          ...
          <div><div style={s.heroH1}>애매한 운세는 없다<br/>확실한 답만 드립니다</div></div>
        </div>
      ...
      <div style={s.section}>
        ...
        <div style={s.calRow}>
          <button onClick={() => set('calendar','solar')} style={{ ...s.calBtn, ...(form.calendar==='solar' ? s.calBtnOn : {}) }}>양력</button>
          <button onClick={() => set('calendar','lunar')} style={{ ...s.calBtn, ...(form.calendar==='lunar' ? s.calBtnOn : {}) }}>음력</button>
          {form.calendar === 'lunar' && (
            <label style={s.leapLabel}><input type="checkbox" checked={form.is_leap} onChange={e => set('is_leap', e.target.checked)} style={{ marginRight:4 }} />윤달</label>
          )}
        </div>
        <div style={s.row3}>
          <input type="number" placeholder="출생 연도" value={form.year} onChange={e => set('year', e.target.value)} style={s.input} />
        ...
        <select value={form.hour} onChange={e => set('hour', e.target.value)} style={{ ...s.input, width:'100%', marginBottom:7 }}>
          <option value="">시주 선택 (자시 ~ 해시)</option>
          {HOURS.map((h,i) => <option key={i} value={i}>{h}</option>)}
        </select>
        <div style={s.sectionLabel}>👤 성별</div>
        <div style={s.row2}>
          <button onClick={() => set('is_male','false')} style={{ ...s.genderBtn, ...(form.is_male==='false' ? s.genderBtnOn : {}) }}>여성 ♀</button>
          <button onClick={() => set('is_male','true')} style={{ ...s.genderBtn, ...(form.is_male==='true' ? s.genderBtnOnMale : {}) }}>남성 ♂</button>
        </div>
      </div>
      <div style={s.divider} />
      <div style={s.svcSection}>
        ...
        <div style={s.svcRow}>
          <div style={s.svcCardFeatured}>
            ...
            <select style={{ ...s.input, fontSize:10, padding:'6px 7px', marginBottom:6 }} value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))}>
              {[2026,2027,2028,2029,2030].map(y => <option key={y} value={y}>{y}년 운세</option>)}
            </select>
            <div style={{ flex:1 }} />
          ...
          <div style={s.svcCard}>
            ...
            <div style={s.svcPrice}>1,990<span style={s.unit}>원</span></div>
            <div style={{ flex:1 }} />
            <button style={s.ctaBtnOutline} onClick={handleYukim}>하늘에 묻기</button>
          </div>
        </div>
      </div>
      <div style={s.ddayBox}>
      ...
      <div style={s.linkGrid}>
        {LINKS.map(l => (
          <a key={l.name} href={l.url} target="_blank" rel="noreferrer" style={s.linkItem}>
            ...
            <div style={{ flex:1, minWidth:0 }}><div style={s.linkName}>{l.name}</div><div style={s.linkSub}>{l.sub}</div></div>
...
const s = {
  ...
  loadingOverlay: { position:'fixed', inset:0, zIndex:200, background:'rgba(255,255,255,0.92)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center' },
  orb: { width:64, height:64, borderRadius:'50%', background:'radial-gradient(circle at 35% 35%, #f3dca6, #d4537e 55%, #7c3aed 100%)', marginBottom:16, position:'relative', animation:'pulseOrb 1.6s ease-in-out infinite' },
  orbRing: { position:'absolute', inset:-8, borderRadius:'50%', border:'2px solid rgba(212,83,126,0.3)', animation:'ringPulse 1.6s ease-out infinite' },
